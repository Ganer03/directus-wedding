...
var AISecret = import_v36.z.object({
  ...
  secret_updated_at: import_v36.z.union([import_v36.z.string(), import_v36.z.null()]).optional(),
  ...
  secret_updated_by_user_id: import_v36.z.union([import_v36.z.string(), import_v36.z.null()]).optional(),
...
var ChatCompletionMessageParam = import_v36.z.union([
  ...
  import_v36.z.object({
    ...
    reasoning_signature: import_v36.z.string().optional()
  }),
  import_v36.z.object({
    content: import_v36.z.union([import_v36.z.string(), import_v36.z.array(ChatCompletionContentPartText)]),
...
var BarProgressReporter = class {
  ...
  constructor() {
    this.multiBar = new cliProgress.MultiBar(
      {
        format: `${(0, import_node_util.styleText)("blueBright", "{bar}")} ${(0, import_node_util.styleText)("blue", "{evaluator}")} {percentage}% ${(0, import_node_util.styleText)("gray", "{value}/{total} {eta_formatted}")}`,
        hideCursor: true,
        barsize: 10
      },
...
function visibleLength(text) {
  return (0, import_node_util2.stripVTControlCharacters)(text).length;
}
function padEndVisible(text, targetLength) {
  return text + " ".repeat(Math.max(0, targetLength - visibleLength(text)));
}
function formatSummaryBox(content) {
  const title = (0, import_node_util2.styleText)("gray", " Experiment summary ");
  ...
  const horizontal = "\u2500";
  const top = (0, import_node_util2.styleText)("gray", "\u256D") + title + (0, import_node_util2.styleText)(
    "gray",
    horizontal.repeat(contentWidth - visibleLength(title)) + "\u256E"
  );
  const body = lines.map(
    (line) => `${(0, import_node_util2.styleText)("gray", "\u2502")} ${padEndVisible(line, contentWidth - 2)} ${(0, import_node_util2.styleText)("gray", "\u2502")}`
  ).join("\n");
  const bottom = (0, import_node_util2.styleText)("gray", "\u2570" + horizontal.repeat(contentWidth) + "\u256F");
  return top + "\n" + body + "\n" + bottom;
}
function formatExperimentSummaryFancy(summary) {
  let comparisonLine = "";
  if (summary.comparisonExperimentName) {
    comparisonLine = `${summary.comparisonExperimentName} ${(0, import_node_util2.styleText)("gray", "(baseline)")} \u2190 ${summary.experimentName} ${(0, import_node_util2.styleText)("gray", "(comparison)")}
...
`;
  ...
  const hasComparison = !!summary.comparisonExperimentName;
  if (hasScores || hasMetrics) {
    const headers = [(0, import_node_util2.styleText)("gray", "Name"), (0, import_node_util2.styleText)("gray", "Value")];
    if (hasComparison) {
      headers.push(
        (0, import_node_util2.styleText)("gray", "Change"),
        (0, import_node_util2.styleText)("gray", "Improvements"),
        (0, import_node_util2.styleText)("gray", "Regressions")
      );
    }
    ...
    for (const score of scoreValues) {
      const scorePercent = (score.score * 100).toFixed(2);
      const scoreValue = (0, import_node_util2.styleText)("white", `${scorePercent}%`);
      let diffString = "";
      if (!isEmpty2(score.diff)) {
        const diffPercent = (score.diff * 100).toFixed(2);
        const diffSign = score.diff > 0 ? "+" : "";
        const diffColor = score.diff > 0 ? "green" : "red";
        diffString = (0, import_node_util2.styleText)(diffColor, `${diffSign}${diffPercent}%`);
      } else {
        diffString = (0, import_node_util2.styleText)("gray", "-");
      }
      const improvements = score.improvements > 0 ? (0, import_node_util2.styleText)(["dim", "green"], String(score.improvements)) : (0, import_node_util2.styleText)("gray", "-");
      const regressions = score.regressions > 0 ? (0, import_node_util2.styleText)(["dim", "red"], String(score.regressions)) : (0, import_node_util2.styleText)("gray", "-");
      const row = [`${(0, import_node_util2.styleText)("blue", "\u25EF")} ${score.name}`, scoreValue];
      if (hasComparison) {
    ...
    for (const metric of metricValues) {
      const fractionDigits = Number.isInteger(metric.metric) ? 0 : 2;
      const formattedValue = metric.metric.toFixed(fractionDigits);
      const metricValue = (0, import_node_util2.styleText)(
        "white",
      ...
      const improvements = metric.improvements > 0 ? (0, import_node_util2.styleText)(["dim", "green"], String(metric.improvements)) : (0, import_node_util2.styleText)("gray", "-");
      const regressions = metric.regressions > 0 ? (0, import_node_util2.styleText)(["dim", "red"], String(metric.regressions)) : (0, import_node_util2.styleText)("gray", "-");
      const row = [`${(0, import_node_util2.styleText)("magenta", "\u25EF")} ${metric.name}`, metricValue];
      if (hasComparison) {
        row.push(diffString, improvements, regressions);
      }
      combinedTable.push(row);
...
var warning2 = (text) => (0, import_node_util2.styleText)("yellow", text);
var fancyReporter = {
...
async function patchStreamedTurn(streamedTurn, state) {
  ...
  if (turnRecord[PATCHED_STREAMED_TURN] || !isAsyncIterable4(turnRecord.events)) {
    return;
  }
  try {
  ...
  try {
    for await (const event of events) {
      try {
        ...
        logInstrumentationError("OpenAI Codex stream event", error2);
      }
      yield event;
    }
    await finalizeCodexRun(state);
  } catch (error2) {
    await finalizeCodexRun(state, { error: error2 });
    throw error2;
  }
}
...
async function finalizeCodexRun(state, params = {}) {
  ...
  try {
    ...
    safeLog(state.span, {
      ...error2 ? { error: error2 instanceof Error ? error2.message : String(error2) } : {},
...
async function finishActiveLlmSpan(state, error2) {
...
function safeLog(span, event) {
  try {
    ...
    logInstrumentationError("OpenAI Codex span log", error2);
  }
}
...
function bindClaudeLocalToolContextToAsyncIterable(result, localToolContext) {
  if (!isAsyncIterable5(result) || Object.isFrozen(result) || Object.isSealed(result)) {
    return result;
  }
  const stream = result;
  ...
  return {
    ...
    async onDelta(args) {
      try {
        ...
        logInstrumentationError2("Cursor SDK onDelta", error2);
      }
      if (originalOnDelta) {
    ...
    async onStep(args) {
      try {
        ...
        logInstrumentationError2("Cursor SDK onStep", error2);
      }
      if (originalOnStep) {
  ...
  try {
    for await (const message of stream) {
      try {
        ...
        logInstrumentationError2("Cursor SDK stream", error2);
      }
      yield message;
    }
    await finalizeCursorRun(state);
...
function safeLog2(span, event) {
  try {
    ...
    logInstrumentationError2("Cursor SDK span log", error2);
  }
}
...
function patchGenerateStreamResult(result, span, startTime) {
  if (!isObject(result) || !isAsyncIterable3(result.stream)) {
    return false;
  }
  let firstChunkTime;
  ...
  return (callback) => {
    ...
    void (async () => {
      try {
        ...
        span.log({ error: errorMessage(error2) });
      } finally {
        span.end();
      }
    })();
...
function configValue(config3, key) {
  return isObject(config3) ? config3[key] : void 0;
}
...
function handleToolComplete(state, toolCallId, success, result, error2) {
  ...
  try {
    if (!success && error2) {
      tool.span.log({ error: error2.message ?? "tool execution failed" });
    } else {
...
function handleSubagentFailed(state, toolCallId, error2) {
  const subAgent = state.subAgents.get(toolCallId);
  if (!subAgent) {
    return;
  }
  try {
    subAgent.span.log({ error: error2 });
  } finally {
    subAgent.span.end();
    state.subAgents.delete(toolCallId);
  }
}
function handleSessionEnd(state, reason, error2) {
  try {
    ...
    if (error2) {
      state.session.span.log({ error: error2, metadata: sessionMetadata });
    } else {
...
function injectTracingHooks2(config3, state) {
  const existingHooks = config3.hooks ?? {};
...
function makeSessionHandlers(sessionStates, configArgIndex, includeProviderMetadata) {
  return {
    start: (event) => {
      const config3 = event.arguments[configArgIndex];
      ...
      if (includeProviderMetadata && config3.provider?.type) {
        metadata["github_copilot.provider_type"] = config3.provider.type;
      }
      if (Object.keys(metadata).length > 0) {
      ...
      injectTracingHooks2(config3, state);
...
function evalWithModuleContext(inFile, evalFn) {
  ...
  try {
    module.paths = nodeModulesPaths(import_node_path.default.dirname(inFile), {});
    return evalFn();
  } finally {
...
function loadModule({
  ...
  return evalWithModuleContext(inFile, () => {
    ...
    const state = _internalGetGlobalState();
    globalThis[/* @__PURE__ */ Symbol.for("braintrust-state")] = state;
...
async function makeSourceMapContext({
  ...
  return {
    ...
    sourceMapDir: import_node_path2.default.dirname(sourceMapFile),
    sourceMap
  };
}
...
async function findCodeDefinition({
  ...
  if (!inFiles[originalPosition.source]) {
    const originalFile = import_node_path2.default.join(sourceMapDir, originalPosition.source);
...
async function uploadBundles({
  ...
  const bundleFile = import_node_path3.default.resolve(bundleFileName);
...
function safeStringify(obj) {
  try {
    ...
    try {
      return import_node_util4.default.inspect(obj, {
        depth: 5,
        maxStringLength: 1e3,
        breakLength: 80,
        compact: false
...
async function initExperiment2(evaluator, evaluatorData) {
  ...
  const linkText = info.experimentUrl ? (0, import_termi_link2.terminalLink)(info.experimentUrl, info.experimentUrl, {
    fallback: (_text, url) => url
  }) : "locally";
  console.error(
    (0, import_node_util5.styleText)("cyan", "\u25B6") + ` Experiment ${(0, import_node_util5.styleText)("bold", info.experimentName)} is running at ${linkText}`
  );
  return logger;
}
...
async function runOnce(handles, opts) {
  ...
  console.error(
    (0, import_node_util5.styleText)(
      "dim",
      `Processing ${(0, import_node_util5.styleText)("bold", String(resultPromises.length))} evaluator${resultPromises.length === 1 ? "" : "s"}...`
    )
  );
...
function checkMatch(pathInput, include_patterns, exclude_patterns) {
  const p = import_node_path5.default.resolve(pathInput);
...
async function collectFiles(inputPath, mode) {
  ...
  if (!pathStat.isDirectory()) {
    ...
    const walked = await import_node_util5.default.promisify(fsWalk.walk)(inputPath, {
      deepFilter: (entry) => {
...
async function initializeHandles({
  ...
  for (const inputPath of inputPaths) {
    ...
    for (const file of newFiles) {
      files[import_node_path5.default.resolve(file)] = true;
    }
  }
  ...
  const tmpDir = import_node_path5.default.join(import_node_os.default.tmpdir(), `btevals-${(0, import_uuid3.v4)().slice(0, 8)}`);
  const initPromises = [];
  for (const file of Object.keys(files)) {
    const baseName = `${import_node_path5.default.basename(
      file,
      import_node_path5.default.extname(file)
    )}-${(0, import_uuid3.v4)().slice(0, 8)}`;
    const outFile = import_node_path5.default.join(tmpDir, `${baseName}.${OUT_EXT}`);
    const bundleFile = import_node_path5.default.join(tmpDir, `${baseName}.bundle.js`);
    initPromises.push(
      initFile({
        inFile: file,
        outFile,
